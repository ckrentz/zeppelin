...
export ZEPPELIN_JAVA_OPTS="-Dspark.executor.memory=4g -Dspark.cores.max=16"      		# Additional jvm options. for example, export ZEPPELIN_JAVA_OPTS="-Dspark.executor.memory=8g -Dspark.cores.max=16"
export ZEPPELIN_MEM="-Xms4096m -Xmx4096m -XX:MaxPermSize=2048m"            		# Zeppelin jvm mem options Default -Xms1024m -Xmx1024m -XX:MaxPermSize=512m
export ZEPPELIN_INTP_MEM="-Xms4096m -Xmx4096m -XX:MaxPermSize=2048m"       		# zeppelin interpreter process jvm mem options. Default -Xms1024m -Xmx1024m -XX:MaxPermSize=512m
...
##
export SPARK_HOME=/opt/spark/spark-current                             # (required) When it is defined, load it instead of Zeppelin embedded Spark libraries
export SPARK_SUBMIT_OPTIONS="--driver-memory 4G --executor-memory 6G"                   # (optional) extra options to pass to spark submit. eg) "--driver-memory 512M --executor-memory 1G".
...
export HADOOP_CONF_DIR=/opt/hadoop/hadoop-current/etc/hadoop
